...
def tourism_based_on_budget(city, budget):
    cityInfo = []
    for info in trip_db:
        if info.city == city and info.type == "Tourism":
            review = 0
            try:
...
def restaurant_based_on_budget(city, budget):
    cityInfo = []
    for info in trip_db:
        if info.city == city and info.type == "Restaurant":
            review = 0
            try:
                review = int(info.review)
            except:
...
output = tourism_based_on_budget("Beijing", "100")
# output = restaurant_based_on_budget("Beijing", "100")
for ele in output:
    print(ele.city, ele.name, ele.price, ele.review)